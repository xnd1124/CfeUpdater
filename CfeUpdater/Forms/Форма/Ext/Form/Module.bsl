...
Процедура ПоказатьОтличия(СтрокаРазбора)  
	Если Не ЗначениеЗаполнено(ПутьКПрограммеСравнения) Тогда
		ПоказатьПредупреждение(, "В настройках не указан путь к программе запуска");
		Возврат;
	КонецЕсли;   
...
&НаКлиенте
Процедура ПослеЗапускаПрограммыСравнения(Результат, ДопПараметрыЗапуска) Экспорт  
	ЭтотОбъект.ПрограммаСравненияЗапущена = Ложь;
    УдалитьФайлы(ДопПараметрыЗапуска.ИмяОсновная);
	УдалитьФайлы(ДопПараметрыЗапуска.ИмяРасширение);
	Если ДопПараметрыЗапуска.Свойство("ИмяНовая") Тогда
		УдалитьФайлы(ДопПараметрыЗапуска.ИмяНовая);
	КонецЕсли;
...
Процедура ПутьКПрограммеСравненияПриИзменении(Элемент)
	...
	Если СтрНайти(ПутьКПрограммеСравнения, "kdiff") > 0 Тогда
		ЭтотОбъект.ПараметрыКоманднойСтроки3Файла = "[СтараяКонфигурация] [Расширение] [НоваяКонфигурация] -m --L1 [ЗаголовокСтарая] --L2 [ЗаголовокРасширение] --L3 [ЗаголовокНовая]";
		ЭтотОбъект.ПараметрыКоманднойСтроки2Файла = "[СтараяКонфигурация] [Расширение]  --L1 [ЗаголовокСтарая] --L2 [ЗаголовокРасширение]";
	...
	Если СтрНайти(ПутьКПрограммеСравнения, "p4merge") > 0 Тогда
		ЭтотОбъект.ПараметрыКоманднойСтроки3Файла = "-C utf8-bom -dw -nb [ЗаголовокСтарая] -nl [ЗаголовокРасширение] -nr [ЗаголовокНовая] [СтараяКонфигурация] [Расширение] [НоваяКонфигурация] ";
		ЭтотОбъект.ПараметрыКоманднойСтроки2Файла = "-C utf8-bom -dw -nl [ЗаголовокСтарая] -nr [ЗаголовокРасширение]  [СтараяКонфигурация] [Расширение]";
