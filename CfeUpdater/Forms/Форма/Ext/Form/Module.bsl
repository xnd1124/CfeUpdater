...
&НаКлиенте
Процедура ПоказатьОтличия(СтрокаРазбора, БезОбъединения = Ложь)
	Если НЕ ЗначениеЗаполнено(Объект.ПрограммаПросмотраИзменений) Тогда
	...
	ЗапуститьПрограммуСравнения(ПараметрыКоманднойСтроки, РезультатСоздания.ЗначенияПараметров,
		РезультатСоздания.ДопПараметрыЗапуска);
КонецПроцедуры
...
Процедура ОбработкаВыбораСтрокиДерева(ВыбраннаяСтрока, ИмяДерева, БезОбъединения = Ложь)
	...
	ПоказатьОтличия(СтрокаРазбора, БезОбъединения);
КонецПроцедуры
