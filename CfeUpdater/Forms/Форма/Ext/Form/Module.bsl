...
Асинх Процедура ПослеЗапускаПрограммыАвтоОбъединения(Результат, ДопПараметрыЗапускаПриложения) Экспорт
	...
	Если СтрНайти(РезультатАнализа, МаркерКонфликта) = 0 Тогда
		...
		ТекущаяСтрока.РезультатОбъединения = РезультатАнализа;
	КонецЕсли;
...
Процедура ЗаменитьАвтоОбновляемыеМетоды()
	...
	ШаблонТекстаЗамены = "%1%2%3";
	
	Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
		МетодРасширение = НайденнаяСтрока.МетодРасширение;
		...
		ЧемЗаменяем = СтрШаблон(ШаблонТекстаЗамены, МетодРасширение.ШапкаМетода,
				НайденнаяСтрока.РезультатОбъединения, МетодРасширение.ЗавершениеМетода);
		
		СодержимоеРасширения = СодержимоеФайла(НайденнаяСтрока.ФайлРасширения);
		НовоеСодержимое = ЗаменаПоШаблону(ЧтоИщем, СодержимоеРасширения, ЧемЗаменяем);
		Если СодержимоеРасширения = НовоеСодержимое Тогда
			Текст = "Произошла ошибка при автоматическом объединении метода " + НайденнаяСтрока.НовыйМетод;
			ОбщегоНазначенияКлиент.СообщитьПользователю(Текст);
			НайденнаяСтрока.Авто = Ложь;
		Иначе
			ИмяФайла = СтрЗаменить(НайденнаяСтрока.ФайлРасширения, ФайлыРасширения, КаталогСРезультатами);
