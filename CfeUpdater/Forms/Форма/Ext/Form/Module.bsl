...
Функция РезультатСравненияМетодов(Строка) 
	...
	ИначеЕсли КоличествоАргументов(СтарыйМетод) <> КоличествоАргументов(НовыйМетод) Тогда
		Возврат "Отличается количество аргументов"; 
	КонецЕсли;                        
	СтароеТело = ТелоМодуля(СтарыйМетод);
	НовоеТело = ТелоМодуля(НовыйМетод);
	Если СтароеТело <> НовоеТело Тогда
		Возврат ТекстОтличаютсяТекстыМетодов();   
	КонецЕсли;           
	
	Если Строка.Аннотация = "ИзменениеИКонтроль" Тогда
		ТелоБезВставикУдаления = ТелоБезВставкиУдаления(РасширенияМетод); 
		Если ТелоБезВставикУдаления <> НовоеТело Тогда
