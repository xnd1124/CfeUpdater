...
Процедура ПриОткрытии(Отказ)
	...
	Исключение
		ПоказатьПредупреждение( , "Не удалось инициализировать работу с регулярными выражениями (VBScript.RegExp). 
			|(Поддерживается работа только в ОС Windows");
		Отказ = Истина;
	КонецПопытки;
	ИмяФайлаРасширения = "";
КонецПроцедуры
...
Функция ПоискПоШаблону(ШаблонПоиска, ГдеИскать)
	РВ.Pattern = ШаблонПоиска;
	НайденныеФрагменты = Новый Массив;
	НайденныеМеста = РВ.Execute(ГдеИскать);
	Для Счет = 0 По НайденныеМеста.Count() - 1 Цикл
		НайденныйУчасток = НайденныеМеста.Item(Счет);
		Фрагменты = НайденныйУчасток.SubMatches;
		РезультатПоиска = Новый Структура("Значение,Группы");
		РезультатПоиска.Значение = НайденныйУчасток.Value;
		НайденныеГруппы = Новый Массив;
		Для СчетВнутренний = 0 По Фрагменты.Count() - 1 Цикл
			НайденныеГруппы.Добавить(Фрагменты.Item(СчетВнутренний));
		КонецЦикла;
...
Функция РезультатСравненияМетодов(ПервыйМетод, ВторойМетод)
	...
	ИначеЕсли ТелоМодуля(ПервыйМетод) <> ТелоМодуля(ВторойМетод) Тогда
		Возврат "Отличаются тексты модулей";
	Иначе
		Возврат "";
	КонецЕсли;
КонецФункции // ()
