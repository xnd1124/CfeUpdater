...
&НаСервере
Функция СокращенныйПутьКМетаданным(Путь, РазделительПути)   
	Результат = СтрЗаменить(Путь, ФайлыРасширения, "");
	Для каждого Сокращение Из СписокСокращений Цикл
		ИскомыйФрагмент = СтрШаблон(Сокращение.Фрагмент, РазделительПути); 	
...
Функция ОбработанныйПутьКМетаданным(Путь) 
	РазделительПути = ПолучитьРазделительПути();
	Результат = СокращенныйПутьКМетаданным(Путь, РазделительПути);    
	Шаблон = "%1%2"; 
...
Функция КартинкаПоИмени(Имя)   
	...
	ИначеЕсли  Имя = "Модуль объекта" Тогда   
		Результат.Вставить("Картинка", БиблиотекаКартинок.РежимПросмотраСпискаСписок); 	
	ИначеЕсли  Имя = "Модуль менеджера" Тогда   
		Результат.Вставить("Картинка", БиблиотекаКартинок.РежимПросмотраСпискаСписок); 		
