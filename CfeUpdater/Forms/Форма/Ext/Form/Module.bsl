...
Процедура ПутьКФайламНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДиалогВыбораФайла.Каталог = КаталогИзПолногоПутиФайла(ЭтотОбъект[Элемент.Имя]);
...
Процедура ПроверитьВозможностьАвтоОбновления()     
	...
	Для каждого Строка Из НайденныеСтроки Цикл   
		...
		ШаблонЗапуска = "git merge-file ""%1"" ""%2"" ""%3"" -p";  
		СтрокаЗапуска = СтрШаблон(ШаблонЗапуска,  
			ИмяФайлаДляРасширения,
			ИмяФайлаДляОсновной,
			ИмяФайлаДляНовой);   
		//для araxis
		//ШаблонЗапуска = """C:\Program Files\Araxis\Araxis Merge\ConsoleCompare.exe"" /a2  /3 /testConflicts /nowait /merge ""%1"" ""%2"" ""%3""";
		//СтрокаЗапуска = СтрШаблон(ШаблонЗапуска,  
		//	ИмяФайлаДляРасширения,
		//	ИмяФайлаДляОсновной,
		//	ИмяФайлаДляНовой);    
...
Асинх Процедура ПослеЗапускаПрограммыАвтоОбъединения(Результат, ДопПараметрыЗапускаПриложения) Экспорт      
	...
	//Если Лев(РезультатАнализа,1) = "0" Тогда
...
Процедура УстановитьТолькоПросмотр(Значение)
	...
	Элементы.ДеревоРазбора.ТолькоПросмотр = Значение;
